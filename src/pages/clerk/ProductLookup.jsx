...
export default function ProductLookup() {
  ...
  const { inventory, movements } = useWarehouse();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Simulate barcode scans, view detailed physical properties, and trace inventory spatial logs.
          </p>
        </div>
      </div>
      ...
      <Card className="border border-gray-100 shadow-sm">
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <Button className="w-full sm:w-auto px-6" onClick={() => {}}>
              Simulate Scanner Gun
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {!selectedProduct ? (
        <div className="h-[40vh] flex flex-col items-center justify-center text-gray-400 gap-2 border-2 border-dashed border-gray-200 rounded-2xl bg-white p-6">
          ...
          <p className="text-xs text-gray-500 text-center max-w-sm">
            Please choose a product from the registry scanner above to view its real-time digital passport.
          </p>
        </div>
      ) : (
              ...
              <CardContent className="p-6 md:p-8 space-y-6">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="bg-white/10 px-3 py-1.5 rounded-xl border border-white/10 text-right">
                    ...
                    <div className="text-xs font-bold text-emerald-400 mt-0.5">{selectedProduct.status}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-slate-50 border border-gray-100 rounded-2xl flex flex-col justify-between">
                    ...
                    <p className="text-[11px] text-gray-500 mt-4 leading-normal">
                      Allocated pick bin. Fits up to 150 standard-sized containers.
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 border border-gray-100 rounded-2xl flex flex-col justify-between">
                    ...
                    <span className="text-xs text-gray-600 font-medium mt-4">
                      Shelf Level: <span className="font-bold text-gray-900">{selectedProduct.shelf}</span>
                    </span>
                  </div>

                </div>

                {/* Pathfinding simulation indicator */}
                <div className="mt-6 p-4 bg-blue-50/20 border border-blue-100/50 rounded-xl flex items-center justify-between text-xs">
                  <div>
                    <h4 className="font-bold text-gray-900">AI Pathfinding Optimization</h4>
                    <p className="text-gray-500 mt-0.5">Average dispatch dock distance from this bin is 45 meters (3 mins walk).</p>
                  </div>
                  <Badge variant="success" className="h-fit">Optimized</Badge>
                </div>
              </CardContent>
            </Card>
